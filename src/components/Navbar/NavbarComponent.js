...
render() {
    ...
    let gHash = MD5( email.trim().toLowerCase() );
    let gravatarURL = "https://www.gravatar.com/avatar/" + gHash
    ...
    return (
        ...
        <nav id='UserNav'>
          {OpenClose}
          <li><Link to="/me"><img src={gravatarURL} style={{width:"25px"}}/></Link></li>
        </nav>
		 	</header>
    );
  }
...
export default createFragmentContainer(Navbar, {
  user: graphql`
    fragment NavbarComponent_user on User {
      authenticated
      email
    }
  `,
